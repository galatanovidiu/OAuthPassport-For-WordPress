...
const SettingsTab = () => {
		...
		try {
			...
			setSettings( {
				...
				oauth_passport_access_token_lifetime:
					response.oauth_passport_access_token_lifetime ?? 3600,
				oauth_passport_refresh_token_lifetime:
					response.oauth_passport_refresh_token_lifetime ?? 2592000,
			} );
		} catch ( error ) {
	...
	const saveSettings = async () => {
		...
		try {
			...
			setSaveMessage(
				__( 'Settings saved successfully!', 'oauth-passport' )
			);
		} catch ( error ) {
			setSaveMessage(
				__(
	...
	return (
		<div className="oauth-settings">
			...
			<Panel>
				<PanelBody
					title={ __( 'OAuth Configuration', 'oauth-passport' ) }
					initialOpen={ true }
				>
					<PanelRow>
						<ToggleControl
							...
							help={ __(
								'Enable OAuth 2.1 authentication for WordPress REST API',
								'oauth-passport'
							) }
					...
					<PanelRow>
						<RangeControl
							...
							value={
								settings.oauth_passport_access_token_lifetime
							}
							onChange={ ( value ) =>
								setSettings( {
									...settings,
									oauth_passport_access_token_lifetime: value,
								} )
							}
							min={ 300 }
							max={ 86400 }
					...
					<PanelRow>
						<RangeControl
							...
							value={
								settings.oauth_passport_refresh_token_lifetime
							}
							onChange={ ( value ) =>
								setSettings( {
									...settings,
					...
					<PanelRow>
						...
						>
							{ saving
								? __( 'Saving...', 'oauth-passport' )
								: __( 'Save Settings', 'oauth-passport' ) }
						</Button>
					</PanelRow>
			...
			<Card>
				<CardHeader>
					<Text variant="title.small">
						{ __( 'OAuth Endpoints', 'oauth-passport' ) }
					</Text>
				</CardHeader>
				<CardBody>
